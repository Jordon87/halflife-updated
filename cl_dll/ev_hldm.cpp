...
//	    SNIPER START
//======================
void EV_Sniper(event_args_t* args)
{
	int idx;
	Vector origin;
	Vector angles;
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/sniper_fire1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));

	EV_GetGunPosition(args, vecSrc, origin);
	VectorCopy(forward, vecAiming);

	EV_HLDM_FireBullets(idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_SNIPER, 2, &tracerCount[idx - 1], args->fparam1, args->fparam2);
}
