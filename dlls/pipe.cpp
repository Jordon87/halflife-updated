...
bool CPipe::GetItemInfo(ItemInfo* p)
{
...
void CPipe::SwingAgain()
{
	Swing(false);
}


bool CPipe::Swing(bool fFirst)
{
	bool fDidHit = false;
	...
	else
	{
		...
		if (fHitWorld)
		{
			...
			{
			case 0:
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/cbar_hit1.wav", fvolbar, ATTN_NORM, 0, 75 + RANDOM_LONG(0, 3));
				break;
			case 1:
				EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/cbar_hit2.wav", fvolbar, ATTN_NORM, 0, 75 + RANDOM_LONG(0, 3));
				break;
			}
